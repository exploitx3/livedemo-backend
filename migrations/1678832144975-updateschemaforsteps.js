'use strict'
import moment from 'moment'
import { setupDB, getModels } from '../src/models.js'
import StepViewTypes from '../src/constants/StepViewTypes.js'
import ScreenPopupTypes from '../src/constants/ScreenPopupTypes.js'
import axios from 'axios.js'
import shortHash from 'short-hash.js'
import { ObjectId } from 'mongodb.js'

module.exports.up = function (next) {

  return migrate()
    .then(() => {
      next()
    })
}

module.exports.down = function (next) {
  next()
}

module.exports.up()

function migrate() {


  var Models
  var newLiveDemoId
  var fullPath

  return setupDB()
    .then(conn => getModels(conn))
    .then(ModelsResult => {
      Models = ModelsResult

    })
    .then(() => {

      return Models.Screen.find({ steps: { $exists: true, $not: { $size: 0 } } }).lean()
    })
    .then((screenDocs) => {
      let updateOps = []

      screenDocs.forEach(screenDoc => {

        screenDoc.steps.forEach(step => {

          let newStep = JSON.parse(JSON.stringify(step))
          newStep.view.hotspot = {}
          newStep.view.pointer = {}
          newStep.view.popup = {}


          if (step.view.viewType === 'Modal') {
            newStep.view.viewType = StepViewTypes.POPUP
            newStep.view.popup.type = 'post'
          }

          if (step.view.viewType === 'Pointer') {
            newStep.view.viewType = StepViewTypes.POINTER
          }


          newStep.view.hotspot = {
            frameX: 200,
            frameY: 200,
            placement: step.view.placement
          }

          newStep.view.pointer = {
            selector: step.view.selector,
            selectorLocation: step.view.selectorLocation,
            placement: step.view.placement,
          }

          if (step.view.viewType === 'Form') {
            newStep.view.viewType = StepViewTypes.POPUP
            newStep.view.popup.type = ScreenPopupTypes.FORM
            newStep.view.popup.formId = step.view.formId
          }


            let updateObj = {
              'steps.$.view.popup': newStep.view.popup,
              'steps.$.view.hotspot': newStep.view.hotspot
            }

            if (newStep.view.selectorLocation && newStep.view.selectorLocation.positionX) {
              updateObj = {
                'steps.$.view.pointer': newStep.view.pointer
              }
            }

            updateOps.push({
              updateOne: {
                filter: { _id: screenDoc._id, 'steps._id': step._id },
                update: {
                  $set: updateObj,
                  $unset: {
                    'steps.$.view.selector': 1,
                    'steps.$.view.selectorLocation': 1,
                    'steps.$.view.placement': 1,
                    'steps.$.view.formId': 1,
                    'steps.$.view.frameX': 1,
                    'steps.$.view.frameY': 1,
                  }
                }
              }
            })

        })
      })

      return Models.Screen.bulkWrite(updateOps, { strict: false })

    })
    .then((result) => {
      console.log(result)
      console.log('docs updated')
    })

    .catch(err => {

      console.log(err)

    })

}
